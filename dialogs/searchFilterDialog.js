...
class SearchFilterDialog extends CancelAndHelpDialog {
    constructor(id) {
        super(id || 'searchFilterDialog');
    ...
    async destinationStep(stepContext) {
      ...
      return await stepContext.prompt(CHOICE_PROMPT, {
          prompt: 'What Filter Should I Search By?',
          choices: ChoiceFactory.toChoices(['designee', 'approver', 'division', 'classification'])
      });
    ...
    async filterStep(stepContext) {
      ...
      stepContext.values.filter = stepContext.result.value;
      ...
      await axios.get(process.env.SearchService +'/indexes/'+ process.env.SearchServiceIndex + '/docs?',
        ...
        var itemArrayMetaData = [];
        var itemArrayMetaUnique = [];

        for (var i = 0; i < this.state.reportArray.length; i++)
        {

          if(stepContext.values.filter === 'designee'){
            itemArrayMetaData.push(this.state.reportArray[i].designee)
          }

          if(stepContext.values.filter === 'approver'){
            itemArrayMetaData.push(this.state.reportArray[i].approver)
          }

          if(stepContext.values.filter === 'division'){
            itemArrayMetaData.push(this.state.reportArray[i].division)
          }

          if(stepContext.values.filter === 'classification'){
            itemArrayMetaData.push(this.state.reportArray[i].classification)
          }
        ...
        return await stepContext.prompt(CHOICE_PROMPT, {
            ...
            choices: ChoiceFactory.toChoices(this.state.itemArrayMetaUnique)
        });
     ...
     */
     async resultStep(stepContext) {
       ...
       stepContext.values.filterSelect = stepContext.result.value;
       ...
       this.state.itemArrayMetaUnique = []

       //console.log(stepContext.values.filterSelect)

       //console.log(stepContext.values.filter)

         var self = this;

         var reportnamequery = "'" + String(stepContext.result.value) + "'"

         await axios.get(process.env.SearchService +'/indexes/'+ process.env.SearchServiceIndex + '/docs?',
             { params: {
               'api-version': '2019-05-06',
               'search': '*',
               '$filter': 'metadata_'+stepContext.values.filter+' eq ' + reportnamequery
               },
             headers: {
               'api-key': process.env.SearchServiceKey,
               'ContentType': 'application/json'
...
module.exports.SearchFilterDialog = SearchFilterDialog;
